...
# filter to only wanted loci
loci_to_keep <- trimws(readLines("/home/plm2976/HybPhaser_output/locifiltered.txt"))
cat("Loci in filter file:", length(loci_to_keep), "\n")
targets_name <- targets_name[targets_name %in% loci_to_keep]
cat("Loci after filtering:", length(targets_name), "\n")
...
for(sample in samples){
  ...
  for(consensus_file in consensus_files) {
    ...
    # skip loci not in filtered list
    if (!gene %in% targets_name) next
