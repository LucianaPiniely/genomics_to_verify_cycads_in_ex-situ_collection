...
# load config
if (!(exists("config_file"))) {config_file <- "./config_filtered.txt"}
source(config_file)
...
output_Robjects <- file.path(path_to_output_folder,"00_R_objects_filtered", name_for_dataset_optimization_subset)
...
if(file.exists(file=file.path(output_Robjects,"Table_SNPs.Rds"))){
  ...
  tab_snps <- readRDS(file=file.path(path_to_output_folder,"00_R_objects_filtered/filtered/Table_SNPs.Rds"))
  tab_length <- readRDS(file=file.path(path_to_output_folder,"00_R_objects_filtered/filtered/Table_consensus_length.Rds"))
}
...
# filter gene_names to only wanted loci
loci_to_keep <- trimws(readLines("/home/plm2976/HybPhaser_output/locifiltered.txt"))
gene_names <- gene_names[gene_names %in% loci_to_keep]
...
# skip boxplot if all values are NA
if(any(!is.na(tab_snps_cl2a_nozero))) {
for (i in 1:2){
  if(i==1){
...
for(i in 1:length(colnames(tab_snps_cl2b))){
  ...
  tab_het_ad$bpoftaet[i] <- round(sum(tab_length_cl2b[,i], na.rm = T)/targets_length_cl2b,3)*100
